...
Eigen::VectorXf SpaceTimeMultigrid(Eigen::VectorXf w_old, Eigen::VectorXf f, int n, int i, float mu, float beta, int K, Eigen::VectorXf u){
    ...
    error = SpaceTimeMultigrid(Eigen::VectorXf::Zero(std::get<2>(d_tilda).size()), std::get<2>(d_tilda), R_n, R_i, mu, beta, K-1, u_res);
...
Eigen::VectorXf Multiple_SpaceTimeMultigrid(Eigen::MatrixXf A, Eigen::VectorXf w_old, Eigen::VectorXf f, int n, int i, float mu, float beta, int K, Eigen::VectorXf u_old){
...
Eigen::VectorXf SpaceTimeSinglegrid(Eigen::VectorXf w_old, Eigen::VectorXf f, int n, int i, float mu, Eigen::VectorXf u_old){
    ...
    std::ofstream out_file, residual_out_file;
	...
	Eigen::MatrixXf M = BuildM(n, (i-1), delt, delx, mu, u_old);
        ...
        for(int j=0; j<n+1; j++){
            p.block(j*(i-1), 0, i-1, 1) = mu * w.block((2*j+1)*(i-1), 0, i-1, 1);
	...
	remove("u_old.txt");
	WriteFile(u_old, "u_old");
	
        remove("u.txt");
        for(int j=0; j<p.size() ; j++){
            u(j) = (-1/mu) * p(j) * y(j);
        }
        u.block(0, 0, i-1, 1) = Eigen::VectorXf::Zero((i-1));
    	u.block((i-1)*n, 0, (i-1), 1) = Eigen::VectorXf::Zero((i-1));
        WriteFile(u, "u");
...
for(int j=0; j < n+1; ++j){
        w.block(2*(i-1)*j, 0, (i-1), 1) =  y.block((i-1)*j, 0, (i-1), 1);
        w.block(2*(i-1)*j + (i-1), 0, (i-1), 1) = p.block((i-1)*j, 0, (i-1), 1);    
}
...
Eigen::VectorXf yd_unconst = CSVtoVector("yd_unconst.txt", i-1, n+1);
Eigen::VectorXf f_unconst = CSVtoVector("f_unconst.txt", i-1, n+1);
f = BoundaryConditions(f, Y, P, yd_unconst, f_unconst, i-1, n);

remove("r.txt");
Eigen::VectorXf r = f - A * w;
WriteFile(r, "r");
        
Eigen::VectorXf r_y = Eigen::VectorXf::Zero((i-1)*(n+1));
Eigen::VectorXf r_p = Eigen::VectorXf::Zero((i-1)*(n+1));

for(int j=0; j<n+1; j++){
	r_y.block(j*(i-1), 0, i-1, 1) = r.block((2*j*(i-1)), 0, i-1, 1);
	r_p.block(j*(i-1), 0, i-1, 1) = r.block((2*j+1)*(i-1), 0, i-1, 1);	
}
...
float ry_norm = r_y.norm();
float rp_norm = r_p.norm();

std::cout << "\n Projection cycle Residual: "  << residual_norm << std::endl;   
std::cout << "\n Projection cycle Converged in: "  << j+1 << " Cycles" << std::endl; 

out_file.open("output.txt", std::ios_base::app); 
out_file << "\n Projection cycle Converged in: "  << j+1 << " Cycles "; 
out_file.close();

residual_out_file.open("residual_norms.txt", std::ios_base::app); 
residual_out_file << "\n r_y Residual norm: "  << ry_norm; 
residual_out_file << "\n r_p Residual norm: "  << rp_norm; 
residual_out_file.close();
...
int main(){
    ...
    Eigen::VectorXf y = Eigen::VectorXf::Random((i-1)*(n+1));
    Eigen::VectorXf p = Eigen::VectorXf::Random((i-1)*(n+1));
    
    Eigen::VectorXf w_old = Eigen::VectorXf::Zero(2*(i-1)*(n+1));
    
    remove("w_old.txt");
    for(int j=0; j < n+1; ++j){
        w_old.block(2*(i-1)*j, 0, (i-1), 1) = y.block((i-1)*j, 0, (i-1), 1);
        w_old.block(2*(i-1)*j + (i-1), 0, (i-1), 1) = p.block((i-1)*j, 0, (i-1), 1);  
    }
    WriteFile(w_old, "w_old");
    ...
    u_old.block(0, 0, i-1, 1) = Eigen::VectorXf::Zero((i-1));
    u_old.block((i-1)*n, 0, (i-1), 1) = Eigen::VectorXf::Zero((i-1));
    u_old = Eigen::VectorXf::Zero((i-1)*(n+1)); 
    WriteFile(u_old, "u_old");

    const clock_t begin_time = clock();
    std::ofstream output_file;
    remove("w.txt");
    Eigen::VectorXf w = SpaceTimeSinglegrid(w_old, f, n, i, mu, u_old);
    WriteFile(w, "w");
