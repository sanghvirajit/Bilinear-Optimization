...
// Single grid cycle
Eigen::VectorXf SpaceTimeSinglegrid(Eigen::VectorXf w_old, Eigen::VectorXf f, int n, int i, float mu, Eigen::VectorXf u_old, float u_a, float u_b){
    ...
    while((residual_norm >= tol)){
        ...
        u = ProjectionFormula(u, i, n, u_a, u_b);
...
int main(){
    ...
    // reading vectors Z, F for constrained case
    Eigen::VectorXf z_const = CSVtoVector("z_const.txt", i-1, n+1);
    Eigen::VectorXf f_const = CSVtoVector("f_const.txt", i-1, n+1); 
    ...
    remove("f.txt");
    f = BoundaryConditions(f, Y, P, z_const, f_const, i-1, n);
    WriteFile(f, "f");
    ...
    // projection
    remove("u_projected.txt");
    u_old = ProjectionFormula(u_old, i, n, u_a, u_b);
    WriteFile(u_old, "u_projected");

    const clock_t begin_time = clock();
    std::ofstream output_file;
    remove("w.txt");
    Eigen::VectorXf w = SpaceTimeSinglegrid(w_old, f, n, i, mu, u_old, u_a, u_b);
    WriteFile(w, "w");
